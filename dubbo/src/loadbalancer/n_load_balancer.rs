...
impl<N, M> MkLoadBalancerBuilder<N, M> {
    ...
    pub fn layer<T: Clone>(mk_router_builder: T) -> impl Layer<N, Service = MkLoadBalancerBuilder<N, T>> {
...
where
    ...
    <N as MakeService<DubboConfig, Url>>::Service: Service<Url, Response = Box<dyn InvokerList + Send + Sync + 'static>, Error = StdError>,
    M: MakeService<DubboConfig, Url, MakeError = StdError>,
    <M as MakeService<DubboConfig, Url>>::Future: Send + 'static,
    <M as MakeService<DubboConfig, Url>>::Service: Service<Url, Response = Box<dyn Router + Send + Sync + 'static>, Error = StdError>,
{
    
    type Response = LoadBalancerBuilder<<N as MakeService<DubboConfig, Url>>::Service, <M as MakeService<DubboConfig, Url>>::Service>;
...
where
    ...
    <M as Service<Url>>::Future: Future<Output = Result<M::Response, M::Error>> + Send + 'static,
